...
The dataset has `r ncol(NYC_df)` variables and they are `r ls(NYC_df)`. I have drop the variables that I donot need and convert `entry` and `vending` from character to logical variables. The dimension of the dataset is `r nrow(NYC_df)` rows * `r ncol(NYC_df)` columns. These data are not tidy enough since the route services have not been converted.
...
* Proportion of station entrance without vending is `r filter(NYC_df, entry == T & vending == F) %>% distinct(., line, station_name) %>% nrow() / nrow(distinct(NYC_df,line, station_name))` while proportion of station exit without vending is `r filter(NYC_df, entry == F & vending == F) %>% distinct(., line, station_name) %>% nrow() / nrow(distinct(NYC_df,line, station_name))`.
...
`pols_month_df` contains the years and months of the count, the number of all kinds of republican (governors, senators, and representatives) on the associate date, the number of all kinds of democratic (governors, senators, and representatives) on the associate date, and which partisan the president is from.


`snp_df` contains the years and months related to Standard & Poor’s stock market index (S&P), and the closing values of the S&P stock index on the associated date.


`unemployment_df` contains the years and months of the measurements, and the percentage of unemployment related to the year and the month.
